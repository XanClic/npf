...
int main(int argc, char *argv[])
{
    ...
    struct bmp_header bmph = {
        .type = "BM",
        .sz = (lines * (fh + 1) - 1) * (32 * (fw + 1) - 1) * 3 + sizeof(struct bmp_header) + sizeof(struct bmp_info),
    ...
    size_t line_length = (32 * (fw + 1) * 3) & ~3;
    ...
    while (cle != NULL)
    {
        unsigned cline = cle->chr->num >> 5;

        memset(buf, 0xFF, bufsz);

        while ((cle != NULL) && ((cle->chr->num >> 5) == cline))
        {
            uint8_t *pos = buf + (cle->chr->num & 0x1F) * (fw + 1) * 3;
